...
def market_buy(client, token_id, label, price_hint=None):
    ...
    if DRY_RUN:
        ...
        est_shares = max((amount / entry_est) * FEE_BUFFER, 0.0) if entry_est > 0 else 0
    ...
    try:
        ...
        filled_shares = max(raw_taking, 0.0)
        ...
        if filled_shares <= 0:
            entry_est = float(price_hint or 0) or get_midpoint(client, token_id)
            filled_shares = max((amount / entry_est) * FEE_BUFFER, 0.0) if entry_est > 0 else 0
            filled_price  = float(entry_est)
        elif filled_price <= 0:
            ...
            if attempt == 0 and "not enough balance / allowance" in err:
                ...
                if bal_m and amt_m:
                    ...
                    if 0 < bal_raw < amt_raw:
                        ...
                        retry = max((attempt_shares * ratio) * 0.999, 0.0)
...
def open_position(key, token_id, entry_price, filled_shares=None, window_start=None, is_flip=False):
    if filled_shares is not None and filled_shares > 0:
        net_shares = max(float(filled_shares), 0.0)
    else:
        net_shares = max((BUY_AMOUNT / entry_price) * FEE_BUFFER, 0.0)
