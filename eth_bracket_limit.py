...
RESYNC_INTERVAL_SECS = int(os.getenv("RESYNC_INTERVAL_SECS", "900"))
...
def place_for_date(
    ...
    quiet:           if True, reduce routine logs (used by periodic resync).
    """
    ...
    for bracket, side_label, tok_idx, order_price, order_size in brackets_config:
        ...
        if market is None:
            (log.debug if quiet else log.warning)(f"  Market not found: {slug} — skipping")
            continue
...
def main() -> None:
    ...
    while True:
        ...
        if today not in placed_dates:
            ...
            if time.time() - last_resync_ts >= RESYNC_INTERVAL_SECS:
                try:
                    eth = fetch_eth_spot()
                    upper, upper2, lower, lower2 = get_brackets(eth)
                    st_set_eth(eth, upper, upper2, lower, lower2)
                    ...
                    place_for_date(client, today, eth_brackets, build_slug, allow_place=False, quiet=True)
                    ...
                    if XRP_ENABLED:
                        ...
                        place_for_date(client, today, xrp_brackets, build_xrp_slug, allow_place=False, quiet=True)
                except Exception as e:
