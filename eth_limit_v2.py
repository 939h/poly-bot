...
try:
    ...
    print("Run: pip install py-clob-client-v2 python-dotenv requests colorama")
    sys.exit(1)
...
# ── XRP config ────────────────────────────────────────────────────────────────
XRP_ENABLED                   = os.getenv("XRP_ENABLED", "true").lower() == "false"
...
def place_limit_order(
    ...
    side: Side.BUY or Side.SELL (use BUY / SELL aliases defined at import).
    """
...
def get_open_order_ids(client: ClobClient, condition_id: str) -> set[str] | None:
    ...
    if DRY_RUN:
        return set()
    try:
        open_orders = client.get_open_orders(OpenOrderParams(market=condition_id))
...
def cancel_all_open_sells(
    ...
    if DRY_RUN:
        ...
        return 0
    try:
        orders = client.get_open_orders(OpenOrderParams(market=condition_id))
        count = 0
        for o in (orders or []):
...
def last_hour_sell_monitor(
    ...
    while time.time() < market_end:
        ...
        try:
            client.cancel_order(OrderPayload(orderID=current_oid))
...
def resync_orphan_buys(client: ClobClient, known_order_ids: set[str]) -> list[dict]:
    ...
    if DRY_RUN:
        return []
    try:
        all_open = client.get_open_orders(OpenOrderParams()) or []
    except Exception as e:
