"""
Polymarket 15-Min Up/Down Bot — Fresh v22
Markets: BTC, ETH, SOL
...
ET  = timezone(timedelta(hours=-4)) # ET (EDT, UTC-4) — adjust to -5 for EST in winter

# ── 1H Monitor Settings ───────────────────────────────────────────────────────
MONITOR_ASSETS        = ["btc", "eth", "sol", "xrp"]
MONITOR_POLL_SECS     = 30   # poll every 30 seconds
MONITOR_START_OFFSET  = 1800 # only monitor last 30 min of each 1hr window (1800s)
MONITOR_ASSET_NAMES   = {    # full names for slug building
    "btc": "bitcoin",
    "eth": "ethereum",
    "sol": "solana",
    "xrp": "ripple",
}
MONITOR_SHEET_TABS    = {    # Google Sheet tab names
    "btc": "BTC 1H",
    "eth": "ETH 1H",
    "sol": "SOL 1H",
    "xrp": "XRP 1H",
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("fresh_bot22.log"),
    ],
    force=True,
)
...
_ob_handler = logging.FileHandler("fresh_bot22.log")
...
_1h_sheets    = {}   # asset -> worksheet
_1h_buffers   = {}   # asset -> list of rows
_1h_last_poll = {}   # asset -> last poll timestamp

def get_or_create_1h_sheet(gc_client, asset):
    """Get or create a 1H monitor tab for an asset."""
    tab_name = MONITOR_SHEET_TABS[asset]
    try:
        spreadsheet = gc_client.open_by_key(SHEET_ID)
        try:
            ws = spreadsheet.worksheet(tab_name)
        except Exception:
            ws = spreadsheet.add_worksheet(title=tab_name, rows=10000, cols=4)
        ws.clear()
        ws.update([["time", "yes", "no", "spread"]], "A1")
        log.info(f"  Sheets | {tab_name} tab ready ✓")
        return ws
    except Exception as e:
        log.error(f"1H sheet setup error ({tab_name}): {e}")
        return None

def setup_1h_sheets(gc_client):
    """Initialize all 1H monitor tabs."""
    global _1h_sheets, _1h_buffers
    for asset in MONITOR_ASSETS:
        _1h_sheets[asset]  = get_or_create_1h_sheet(gc_client, asset)
        _1h_buffers[asset] = []
        _1h_last_poll[asset] = 0

def build_1h_slug(asset, ts):
    """Build the Polymarket slug for an hourly market at the given UTC timestamp."""
    dt      = datetime.fromtimestamp(ts, tz=ET)
    hour_dt = dt.replace(minute=0, second=0, microsecond=0)
    name    = MONITOR_ASSET_NAMES.get(asset, asset)
...
def monitor_1h_tick(client, server_ts):
    """Check 1H markets and log prices if in last 30 min of window."""
    global _1h_buffers, _1h_last_poll
    secs_into_hour = server_ts % 3600
    if secs_into_hour < MONITOR_START_OFFSET:
        return  # Not in last 30 min yet

    for asset in MONITOR_ASSETS:
        if server_ts - _1h_last_poll.get(asset, 0) < MONITOR_POLL_SECS:
            continue
        _1h_last_poll[asset] = server_ts

        slug = build_1h_slug(asset, server_ts)
        mkt  = fetch_market_by_slug(slug)
        if not mkt:
            continue
        tokens = get_tokens(mkt)
        if not tokens or len(tokens) < 2:
            continue
        yes_token, no_token = tokens[0], tokens[1]
        yes_p = get_midpoint(client, yes_token)
        no_p  = get_midpoint(client, no_token)
        if yes_p <= 0 or no_p <= 0:
            continue
        spread = round(abs(yes_p - no_p), 4)
        ...
        if ws and len(_1h_buffers[asset]) >= 10:
            try:
                ws.append_rows(_1h_buffers[asset])
                _1h_buffers[asset] = []
            except Exception as e:
                log.error(f"1H sheet flush error ({asset}): {e}")

def flush_all_1h_sheets():
    """Flush all remaining 1H buffer rows to sheets."""
    for asset in MONITOR_ASSETS:
        ws = _1h_sheets.get(asset)
        if ws and _1h_buffers.get(asset):
            try:
                ws.append_rows(_1h_buffers[asset])
                _1h_buffers[asset] = []
            except Exception as e:
                log.error(f"1H final flush error ({asset}): {e}")
...
def check_btc_volatility():
    """
    Fetch latest BTC/USDT 10-min candle from Binance.
    Returns (is_volatile, range_pct) silently — no log unless triggered.
    """
    try:
        r = requests.get(
            BINANCE_API,
            params={"symbol": "BTCUSDT", "interval": "10m", "limit": 1},
            timeout=5
        )
        candle = r.json()[0]
        ...
        return range_pct > VOLATILITY_THRESHOLD, range_pct
    except Exception:
        return False, 0.0


def record_volatility_pause(sheet, range_pct, resume_time):
    """Write ONE row to Sheet1 when volatility pause is triggered."""
    if sheet is None:
        return
    try:
        now_str    = datetime.now(MYT).strftime("%Y-%m-%d %H:%M:%S MYT")
        resume_str = datetime.fromtimestamp(resume_time, MYT).strftime("%Y-%m-%d %H:%M:%S MYT")
        sheet.append_row([
            now_str,
            "VOLATILITY PAUSE",
            f"BTC 10m range: {range_pct:.2%}",
            f"Resume at: {resume_str}",
            "", "", "", "", "", "", "", "", ""
        ])
        log.info(f"  Volatility guard triggered — BTC 10m range={range_pct:.2%} | Paused 4hrs until {resume_str}")
    except Exception as e:
        log.error(f"  Volatility sheet record failed: {e}")
...
def run():
    ...
    log.info(f"Buy {BUY_PRICE_MIN:.0%}-{BUY_PRICE_MAX:.0%} between {ENTRY_AFTER//60}-{STOP_BUY_AT//60}min | Sell @ {SELL_PRICE:.0%} | Cut loss @ 40%")
    ...
    while True:
        try:
            ...
            # ── Volatility Guard ──────────────────────────────────────────
            if server_ts - last_volatility_check >= VOLATILITY_CHECK_INTERVAL:
                last_volatility_check = server_ts
                is_volatile, range_pct = check_btc_volatility()
                if is_volatile and not volatility_paused:
                    volatility_paused    = True
                    volatility_resume_ts = server_ts + VOLATILITY_PAUSE_SECS
                    record_volatility_pause(sheet, range_pct, volatility_resume_ts)

            # Resume after 4 hours — silent
            if volatility_paused and server_ts >= volatility_resume_ts:
                volatility_paused = False
                log.info(f"  Volatility guard lifted — resuming normal trading")
            ...
            for asset in ASSETS:
                ...
                # ── Skip buying if volatility guard is active ─────────────
        ...
        # 1H market monitor
        monitor_1h_tick(client, server_ts)
