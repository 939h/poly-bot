"""
Polymarket 1-Hour Crypto Up/Down — Panic Bot
============================================
Strategy:
...
_file = logging.FileHandler("panic_hour.log", encoding="utf-8")
...
def log_price_to_csv(asset, price):
    ...
    file_path = "data/live_prices_1h.csv"
...
SETTLE_SECS      = 600   # first 600s (10 min) of window = collect prices, no trading
...
HOLD_EARLY_SECS  = 120    # 0-20 min  (early market)  — wait 120s before selling
HOLD_MID_SECS    = 90     # 20-40 min (middle market) — wait 90s before selling
HOLD_LATE_SECS   = 60     # 40-60 min (late market)   — wait 60s before selling

# --- Timing ------------------------------------------------------------------
POLL_SECS        = 1        # seconds between each price scan
STOP_TRADE_SECS  = 3300   # stop opening NEW trades after this many seconds into window
                          # 3300 = 55 minutes  (window is 3600s = 60 min)
...
def main():
    ...
    while True:
        try:
            ...
            # New window — reset everything (each 1h window is independent)
        ...
        if idle_now and not open_positions:
            time.sleep(600)  # outside trading hours + no open positions — check every 10min
        else:
