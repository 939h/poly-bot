...
BUY_PRICE_MIN   = 0.55    # Buy if price >= 80c
BUY_PRICE_MAX   = 0.70    # Buy if price <= 85c
SELL_PRICE      = 0.72     # Sell main shares at 97c
...
def market_buy(client, token_id, shares, price, label):
    ...
    try:
        ...
        actual_shares = math.floor(amount / price * 100) / 100  # fallback estimate
        taking = resp.get("takingAmount") if isinstance(resp, dict) else None
        if taking:
            try:
                actual_shares = math.floor(float(taking) * 100) / 100
            except Exception:
                pass
        log.info(f"  MARKET BUY executed: {label} | actual={actual_shares} shares | {resp}")
