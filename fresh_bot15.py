"""
Polymarket 15-Min Up/Down Bot — Fresh v15
Markets: BTC, ETH, SOL
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("fresh_bot15.log"),
    ],
    force=True,
)
...
_ob_handler = logging.FileHandler("fresh_bot15.log")
...
DRY_RUN         = True
ASSETS          = ["btc", "eth", "sol"]
BUY_SHARES      = 5           # exact shares to buy per trade
BUY_PRICE_MIN   = 0.82        # buy if price >= 82c
BUY_PRICE_MAX   = 0.84        # buy if price <= 84c
SELL_PRICE      = 0.97        # limit sell target price
...
def limit_buy(client, token_id, size, price, label):
    """Place a GTC limit buy order. size = exact shares, price = price per share."""
    if DRY_RUN:
        log.info(f"  [DRY RUN] LIMIT BUY {size} {label} @ {price:.2%} = ${round(size*price,4):.4f} USDC")
        return price, size
    try:
        order = client.create_order(OrderArgs(token_id=token_id, price=price, size=size, side=BUY))
        resp  = client.post_order(order, OrderType.GTC)
        log.info(f"  LIMIT BUY executed: {label} | {size} shares @ {price:.2%} | {resp}")
        return price, size
    except Exception as e:
        log.error(f"  LIMIT BUY failed ({label}): {e}")
        return None, None
...
def run():
    ...
    log.info(f"Buy {BUY_PRICE_MIN:.0%}-{BUY_PRICE_MAX:.0%} between {ENTRY_AFTER//60}-{STOP_BUY_AT//60}min | Sell @ {SELL_PRICE:.0%} | Cut: 50% | {BUY_SHARES} shares per trade (limit orders)")
            ...
            for asset in ASSETS:
                ...
                for pos_key, positions in list(active_positions.items()):
                    ...
                    for pos in list(positions):
                        ...
                        # Flip trade — market sell at 97c+, NO cut loss, hold to resolution
                        if pos.get("is_flip"):
                            if price >= SELL_PRICE:
                                log.info(f"  [{pos_asset.upper()} {side}] FLIP SELL @ {price:.2%}!")
                                size = pos.get("actual_shares") or pnl.trades[idx]["buy_shares"]
                                sp = None
                                try:
                                    fs_order = client.create_market_order(MarketOrderArgs(token_id=token_id, amount=size, side=SELL))
                                    fs_resp  = client.post_order(fs_order, OrderType.FOK)
                                    log.info(f"  FLIP SELL executed: {pos_asset.upper()}-{side} | {size} shares @ {price:.2%} | {fs_resp}")
                                    sp = price
                                except Exception as e:
                                    log.error(f"  FLIP SELL failed ({pos_asset.upper()}-{side}): {e}")
                                if sp is not None:
                        ...
                        # Cut loss — market sell immediately, price crashing fast
                        if price <= cut_loss_price:
                            log.info(f"  [{pos_asset.upper()} {side}] CUT LOSS @ {price:.2%} (bought @ {buy_price:.2%})")
                            size = pos.get("actual_shares") or pnl.trades[idx]["buy_shares"]
                            sp = None
                            try:
                                cl_order = client.create_market_order(MarketOrderArgs(token_id=token_id, amount=size, side=SELL))
                                cl_resp   = client.post_order(cl_order, OrderType.FOK)
                                log.info(f"  CUT-LOSS SELL executed: {pos_asset.upper()}-{side} | {size} shares @ {price:.2%} | {cl_resp}")
                                sp = price
                            except Exception as e:
                                log.error(f"  CUT-LOSS SELL failed ({pos_asset.upper()}-{side}): {e}")
                            if sp is not None:
                                ...
                                if opp_price < 0.50:
                                    ...
                                    # Flip uses market FOK — execute instantly, price moving fast
                                    amount = round(BUY_SHARES * opp_price, 4)
                                    try:
                                        flip_order = client.create_market_order(MarketOrderArgs(token_id=opp_id, amount=amount, side=BUY))
                                        flip_resp  = client.post_order(flip_order, OrderType.FOK)
                                        flip_actual = BUY_SHARES  # limit order shares known exactly
                                        taking = flip_resp.get("takingAmount") if isinstance(flip_resp, dict) else None
                                        if taking:
                                            try:
                                                flip_actual = round(float(taking), 4)
                                            except Exception:
                                                pass
                                        log.info(f"  FLIP BUY executed: {pos_asset.upper()}-{opp_side} | {flip_actual} shares @ {opp_price:.2%} | {flip_resp}")
                                        flip_fill = opp_price
                                    except Exception as e:
                                        log.error(f"  FLIP BUY failed: {e}")
                                        flip_fill = None
                                        flip_actual = 0
                        ...
                        # Sell — market FOK at 97c+, get best available price
                        if price >= SELL_PRICE:
                            log.info(f"  [{pos_asset.upper()} {side}] TRIGGER SELL @ {price:.2%}!")
                            size = pos.get("actual_shares") or pnl.trades[idx]["buy_shares"]
                            sp = None
                            try:
                                ts_order = client.create_market_order(MarketOrderArgs(token_id=token_id, amount=size, side=SELL))
                                ts_resp  = client.post_order(ts_order, OrderType.FOK)
                                log.info(f"  SELL executed: {pos_asset.upper()}-{side} | {size} shares @ {price:.2%} | {ts_resp}")
                                sp = price
                            except Exception as e:
                                log.error(f"  SELL failed ({pos_asset.upper()}-{side}): {e}")
                            if sp is not None:
                                ob_record(pos_asset, get_midpoint(client, pyt), get_midpoint(client, pnt),
                                         f"*** SELL {side} @ {price:.2%} ***")
                                pnl.record_sell(idx, sp, "SOLD-99c")
                ...
                if BUY_PRICE_MIN <= yes_price <= BUY_PRICE_MAX:
                    ...
                    fill, actual_shares = limit_buy(client, yes_token, BUY_SHARES, yes_price, f"{asset.upper()}-YES")
                    if fill is not None:
                    ...
                    fill, actual_shares = limit_buy(client, no_token, BUY_SHARES, no_price, f"{asset.upper()}-NO")
                    if fill is not None:
